...
attrition_data <- readr::read_csv(here::here('data/WA_Fn-UseC_-HR-Employee-Attrition.csv'))
...
attrition_freq_table <- attrition_data_simplified %>% 
    ...
    count(Attrition) %>% 
    group_by(Attrition) %>% 
    mutate(prop = n/nrow(attrition_data))
...
JobSatisfaction_freq_table <- attrition_data_simplified %>% 
    ...
    count(Attrition, JobSatisfaction) %>% 
    group_by(Attrition) %>% 
    mutate(prop = prop.table(n))
...
PerformanceRating_freq_table <- attrition_data_simplified %>% 
    ...
    count(Attrition, PerformanceRating) %>% 
    group_by(Attrition) %>% 
    mutate(prop = prop.table(n))
...
RelationshipSatisfaction_freq_table <- attrition_data_simplified %>% 
    select(Attrition, RelationshipSatisfaction) %>% 
...
data_attrition <- read.csv("WA_Fn-UseC_-HR-Employee-Attrition.csv", stringsAsFactors = FALSE)
data_attrition
```
...
```{r}
set.seed(123)
split <- initial_split(data_attrition_2, prop = 0.7, strata = "Attrition")
...
cor_train_Attrition_Yes_numeric <- cor(train_Attrition_Yes_numeric)
cor_train_Attrition_Yes_numeric

# Graficamos la matriz anterior para visualizar de mejor manera
corrplot(cor_train_Attrition_Yes_numeric)
```
...
```{r}
pred <- predict(nb_model_3, newdata = validation)
confusionMatrix(pred, validation$Attrition)
```
...
![](C:/Users/maali/OneDrive/SwpDocs/UNAB/Docencia/MachineLearningUNAB/Data_Science_2024-10/images/Figura_2_Capitulo_6_Aprendizaje_Supervisado_Naive_Bayes_Arboles_de_Decision.png "")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
ggplot(data = cars_df) + aes(x = cyl, y = hp, color = mpg) +
  geom_point(size = 3, alpha = 0.8) + 
  geom_vline(xintercept =  5, color = "gray", linetype = 2) +
  geom_segment(x = 5, y = 193, xend = 9, yend = 192, color = "gray", linetype = 2) +
  annotate(geom = "text", x = 4.5, y = 200, label = "R1", color = "red") +
  annotate(geom = "text", x = 7, y = 120, label = "R2", color = "blue") +
  annotate(geom = "text", x = 6.5, y = 250, label = "R3", color = "black") +
...
data_ames <- read.csv("ames_dataset.csv", stringsAsFactors = FALSE)
data_ames
```
...
```{r}
set.seed(123)
ames_split <- initial_split(data_ames, prop = 0.7)
ames_train <- training(ames_split)
ames_train
ames_validation <- testing(ames_split)
ames_validation
```
...
ggplot(data = ames_sales_prices) +
  aes(x = Sale_Price, color = set) +
  geom_density()
```
...
ggplot(data = ames_some_predictors_numerics) +
  aes(x = First_Flr_SF, color = set) +
  geom_density()

ggplot(data = ames_some_predictors_numerics) +
  aes(x = Gr_Liv_Area, color = set) +
  geom_density()

ggplot(data = ames_some_predictors_numerics) +
  aes(x = Total_Bsmt_SF, color = set) +
  geom_density()

ames_some_predictors_categorical <- ames_some_predictors[, c("set", "Overall_Qual", "Neighborhood", "Utilities")]

ggplot(data = ames_some_predictors_categorical) +
  aes(x = Overall_Qual, fill = set) +
  geom_bar(position = "dodge2") +
  coord_flip()

ggplot(data = ames_some_predictors_categorical) +
  aes(x = Neighborhood, fill = set) +
  geom_bar(position = "dodge2") +
  coord_flip()

ggplot(data = ames_some_predictors_categorical) +
  aes(x = Utilities, fill = set) +
  geom_bar(position = "dodge2") +
  coord_flip()
```
...
```{r message=FALSE, warning=FALSE, paged.print=TRUE}
dt_model_1 <- rpart(formula = Sale_Price ~.,
                    data = ames_train,
                    method = "anova")
dt_model_1
```
...
![](C:/Users/maali/OneDrive/SwpDocs/UNAB/Docencia/MachineLearningUNAB/Data_Science_2024-10/images/Figura_3_Capitulo_6_Aprendizaje_Supervisado_Naive_Bayes_Arboles_de_Decision.jpg "")
...
```{r}
dt_model_complete <- rpart(formula = Sale_Price ~ .,
                           data    = ames_train,
                           method  = "anova", 
                           control = list(cp = 0, xval = 10))

rpart.plot(dt_model_complete)
plotcp(dt_model_complete)
...
```{r}
dt_model_2 <- rpart(formula = Sale_Price ~ .,
                    data    = ames_train,
                    method  = "anova", 
...
for (i in 1:nrow(hyper_grid)) {
  ...
  models[[i]] <- rpart(formula = Sale_Price ~ .,
                       data    = ames_train,
                       method  = "anova",
...
```{r}
optimal_dt_model <- rpart(formula = Sale_Price ~ .,
                          data    = ames_train,
                          method  = "anova",
...
ggplot(data = Sale_Prices_df) + 
  aes(x = Predicted_Sale_Price, y = Real_Sale_Price) + 
  geom_point()
```
